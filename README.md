...
through the same saturating curve:
...
Both scopes feed the all-cooling fallback: when every allowed route is
cooling for the request's target under either scope, the fallback hands out
the route that can serve **that target** again soonest — the later of its
route and pair deadlines decides. Pair tracking is bounded per route
(1024 targets; expired entries are reclaimed first, then the
soonest-recovering pair is evicted), so a flood of unique refused targets
cannot grow memory without limit. Pair cooldowns survive a reload exactly
like other retained route state, and the tracked targets are
client-controlled strings that never appear in logs or `/status` — only the
summary counts do.
...
route-level SOCKS handshake failures; `targetCooldowns` counts the route's
...
error category, and cooldown for endpoint dial, SOCKS handshake, and
refused connect-target failures.
